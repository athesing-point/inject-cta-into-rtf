document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!rtf) {
    console.error("Failed to find RTF element.");
    return;
  }
  ...
  if (!myElement) {
    console.error("Failed to find CTA element.");
    return;
  }
  ...
  if (blogCategoryElement && blogCategoryElement.textContent.toLowerCase().includes("homeowner stories")) {
    ...
    console.log("CTA insertion disabled due to category.");
    return;
  }
  ...
  if (myElement && rtf) {
    ...
    console.error("CTA or RTF not found check mappings.");
  }
});
